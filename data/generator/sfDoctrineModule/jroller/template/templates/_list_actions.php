<?php if ($actions = $this->configuration->getValue('list.actions')): ?>
  <?php foreach ($actions as $name => $params):
    ...
    <?php if ('_new' == $name): ?>
      <?php $params['ui-icon'] = isset($params['ui-icon']) ? $params['ui-icon'] : 'plus' ?>
      <?php echo $this->addCredentialCondition('[?php echo $helper->linkToNew('.$this->asPhp($params).') ?]', $params)."\n" ?>

    <?php elseif ('_show' == $name): ?>
      <?php $params['ui-icon'] = isset($params['ui-icon']) ? $params['ui-icon'] : 'document' ?>
    ...
    <li class="sf_admin_action_<?php echo $params['class_suffix'] ?> fg-menu-has-icons">
      <?php $params['label'] .= UIHelper::addIcon($params, 'extlink'); ?>
